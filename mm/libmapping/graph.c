...
{
	LM_ASSERT(type < GRAPH_ELTYPE_NUMBER);
...
{
	LM_ASSERT_PRINTF(g->used_vertices < g->n_vertices, "g->n_vertices %u\n", g->n_vertices);
	g->used_vertices++;
...
{
	LM_ASSERT(g->used_edges < g->n_edges);
	g->used_edges++;
	return g->next_edge++;
}
...
edge_t* libmapping_graph_connect_vertices(graph_t *g, vertex_t *src, vertex_t *dest)
{
	...
	LM_ASSERT(src->arity < g->n_vertices);
	LM_ASSERT(dest->arity < g->n_vertices);
